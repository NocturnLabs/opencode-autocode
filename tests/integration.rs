//! Integration tests for opencode-forger
//!
...
fn test_vibe_detects_database() {
    ...
    let forger_dir = output_path.join(".forger");
    fs::create_dir_all(&forger_dir).expect("Failed to create .forger dir");
    ...
    opencode_forger::db::Database::open(&db_path).expect("Failed to create database");
...
fn test_scaffold_preserves_existing_config() {
    ...
    let forger_dir = output_path.join(".forger");
    fs::create_dir_all(&forger_dir).unwrap();
