...
pub fn run_config_tui() -> Result<()> {
    ...
    // Also save/update opencode.json with MCP settings
    let opencode_json_path = Path::new("opencode.json");
    save_opencode_json(&config, opencode_json_path)?;
...
fn save_opencode_json(config: &Config, path: &Path) -> Result<()> {
    // Determine which MCP servers should be enabled based on config
